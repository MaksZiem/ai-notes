...
export default function NotesPage() {
  ...
  const renderGrid = (notes: Note[], pinned = false) => (
    <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
      {notes.map((n) => (
        <NoteCard key={n.id} note={n} pinned={pinned} />
      ))}
    </div>
  );
  ...
  const renderList = (notes: Note[], pinned = false) => (
    <div className="flex flex-col gap-2">
      {notes.map((n) => (
        <NoteCard key={n.id} note={n} pinned={pinned} list />
      ))}
    </div>
  );
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        <div className="flex items-center justify-between px-8 py-5 border-b border-white/[0.06]">
          ...
          <div className="flex items-center gap-3">
            ...
            <button className="flex items-center gap-2 px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-medium rounded-lg transition-colors duration-150">
              <Plus size={15} />
              Nowa notatka
            </button>
          </div>
